...
int ihex_read(FILE *pFile, unsigned char *buf, unsigned int start, unsigned int end)
{
    ...
    // :LLXXXXTTDDDD....DDCC
    // Where LL is the length of the data field and can be as large as 255.  So the maximum size of a record is 9
    // characters for the header, 255 * 2 characters for the data, 2 characters for the Checksum, and an additional 2
    // characters (possibly a Carriage Return and linefeed.  This is then a total an 9 + 510 + 2 + 2 = 523.
