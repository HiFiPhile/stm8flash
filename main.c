...
void print_help_and_exit(const char *name, bool err)
{
    ...
    fprintf(stream,
            "Usage: %s [-c programmer] [-S serialno] [-p partno] [-s memtype] [-b bytes] [-r|-w|-v] <filename>\n",
            name);
...
void dump_pgms(programmer_t *pgms)
{
    ...
    int i;
    for(i = 0; pgms[i].name; i++) fprintf(stderr, "%s\n", pgms[i].name);
}
...
{
    for(unsigned int i = 0; i < strlen(serialno); i++)
    {
...
bool usb_init(programmer_t *pgm, bool pgm_serialno_specified, char *pgm_serialno)
{
    ...
    if(numOfProgrammers > 1 || pgm_serialno_specified)
    {
        ...
        for(i = 0; i < cnt; i++)
        {
            ...
            if(desc.idVendor == pgm->usb_vid && desc.idProduct == pgm->usb_pid)
            {
                ...
                libusb_get_string_descriptor_ascii(tempHandle, desc.iManufacturer, (unsigned char *)vendor,
                                                   sizeof(vendor));
                libusb_get_string_descriptor_ascii(tempHandle, desc.iProduct, (unsigned char *)device, sizeof(device));
                libusb_get_string_descriptor_ascii(tempHandle, desc.iSerialNumber, (unsigned char *)serialno,
                                                   sizeof(serialno));
    ...
    pgm->ctx = ctx;
    if(!pgm->dev_handle)
        spawn_error("Could not open USB device.");
...
int main(int argc, char **argv)
{
    ...
    bool start_addr_specified = false, pgm_specified = false, pgm_serialno_specified = false, part_specified = false,
    ...
    while((c = getopt(argc, argv, "r:w:v:nc:S:p:d:s:b:luV")) != (char)-1)
    {
        ...
        case 'd':
            ...
            break;
        case 'l':
            for(i = 0; stm8_devices[i].name; i++) printf("%s ", stm8_devices[i].name);
            printf("\n");
            exit(0);
        case 'r':
            action = READ;
    ...
    fprintf(stderr, "Due to its file extension (or lack thereof), \"%s\" is considered as %s format!\n", filename,
            fileformat == INTEL_HEX ? "INTEL HEX"
                                    : (fileformat == MOTOROLA_S_RECORD ? "MOTOROLA S-RECORD" : "RAW BINARY"));
    ...
    else if(action == UNLOCK)
    {
        ...
        if(part->read_out_protection_mode == ROP_UNKNOWN)
            spawn_error("No unlocking mode defined for this device. You may need to edit the file stm8.c");
        ...
        else if(part->read_out_protection_mode == ROP_STM8L)
        {
            ...
            spawn_error("Unimplemented unlocking mode");
        /* flashing MCU */
        int sent = pgm->write_range(pgm, part, buf, start, bytes_to_write, memtype);
        if(pgm->reset)
        {
