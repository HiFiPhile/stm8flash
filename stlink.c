...
int stlink_send_cbw(libusb_device_handle *dev_handle, scsi_usb_cbw *cbw)
{
    ...
    int actual;
    int r = libusb_bulk_transfer(dev_handle, (2 | LIBUSB_ENDPOINT_OUT), buf, USB_CBW_SIZE, &actual, 0);
...
int stlink_read_csw(libusb_device_handle *dev_handle, scsi_usb_csw *csw)
{
    ...
    int           recv;
    int           r = libusb_bulk_transfer(dev_handle, (1 | LIBUSB_ENDPOINT_IN), buf, USB_CSW_SIZE, &recv, 0);
...
int stlink_cmd(programmer_t *pgm, int transfer_length, unsigned char *transfer_out, unsigned char flags, int cblength,
               ...)
{
    va_list ap;
    memset(&cbw, 0, sizeof(scsi_usb_cbw));
...
void stlink_init_session(programmer_t *pgm)
{
    int  i;
    char f4_cmd_arg1[] = {
        0x07, 0x07, 0x08, 0x07, 0x04,
    };
    for(i = 0; i < sizeof(f4_cmd_arg1); i++)
    {
        stlink_cmd(pgm, 0, NULL, 0x00, 0x0a, 0xf4, f4_cmd_arg1[i], 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00);
    ...
    stlink_cmd(pgm, 0, NULL, 0x00, 0x03, 0xf4, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00);
    stlink_swim_get_status(pgm);
    stlink_cmd(pgm, 0, NULL, 0x00, 0x0a, 0xf4, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00);
    stlink_swim_get_status(pgm);

    stlink_swim_write_byte(pgm, 0xa0, 0x7f80); // mov 0x0a, SWIM_CSR2 ;; Init SWIM
    stlink_cmd(pgm, 0, NULL, 0x00, 0x0a, 0xf4, 0x08, 0x00, 0x01, 0x00, 0x00, 0x7f, 0x80, 0xa0, 0x00);
    stlink_swim_get_status(pgm);
    stlink_cmd(pgm, 0, NULL, 0x00, 0x0a, 0xf4, 0x06, 0x00, 0x01, 0x00, 0x00, 0x7f, 0x99, 0xa0, 0x00);
    stlink_swim_get_status(pgm);
...
void stlink_finish_session(programmer_t *pgm)
{
    ...
    stlink_cmd(pgm, 0, NULL, 0x00, 0x0a, 0xf4, 0x05, 0x00, 0x01, 0x00, 0x00, 0x7f, 0x80, 0xb6, 0x00);
    stlink_swim_get_status(pgm);
    stlink_cmd(pgm, 0, NULL, 0x00, 0x0a, 0xf4, 0x07, 0x00, 0x01, 0x00, 0x00, 0x7f, 0x80, 0xb6, 0x00);
    stlink_swim_get_status(pgm);
    stlink_cmd(pgm, 0, NULL, 0x00, 0x03, 0xf4, 0x03, 0x01);
    stlink_swim_get_status(pgm);
}
...
{
    unsigned char buf[4];
    stlink_cmd(pgm, 4, buf, 0x80, 0x0a, 0xf4, 0x09, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00);
...
void stlink_swim_srst(programmer_t *pgm)
{
    ...
    stlink_cmd(pgm, 0, NULL, 0x80, 0x0a, 0xf4, 0x08, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00);
    stlink_swim_get_status(pgm);
}
...
int stlink_swim_write_byte(programmer_t *pgm, unsigned char byte, unsigned int start)
{
    ...
    do
    {
        stlink_cmd(pgm, 0, NULL, 0x00, 0x10, 0xf4, 0x0a, 0x00, 0x01, 0x00, 0x00, start2[0], start2[1], byte, 0x00, 0x00,
                   0x00, 0x00, 0x00, 0x00, 0x00);
        usleep(2000);
        // Ready bytes count (always 1 here)
        stlink_cmd(pgm, 4, buf, 0x80, 0x0a, 0xf4, 0x09, 0x00, 0x01, 0x00, 0x00, start2[0], start2[1], byte, 0x00);
...
int stlink_swim_read_range(programmer_t *pgm, const stm8_device_t *device, unsigned char *buffer, unsigned int start,
                           unsigned int length)
{
    ...
    for(i = 0; i < length; i += STLK_READ_BUFFER_SIZE)
    {
        ...
        stlink_cmd(pgm, 0, NULL, 0x80, 0x0a, 0xf4, 0x0b, block_size2[0], block_size2[1], 0x00, 0x00, block_start2[0],
                   block_start2[1], 0x00, 0x00);
        ...
        stlink_cmd(pgm, block_size, &(buffer[i]), 0x80, 0x0a, 0xf4, 0x0c, block_size2[0], block_size2[1], 0x00, 0x00,
                   block_start2[0], block_start2[1], 0x00, 0x00);
    }
...
int stlink_swim_write_block(programmer_t *pgm, unsigned char *buffer, unsigned int start, unsigned int length,
                            unsigned int padding)
{
    ...
    if(length2)
    {
        ...
        if(padding)
            tail[length2] = '\1';
        int actual;
        int r = libusb_bulk_transfer(pgm->dev_handle, (2 | LIBUSB_ENDPOINT_OUT), tail, length2 + padding, &actual, 0);
...
int stlink_swim_write_range(programmer_t *pgm, const stm8_device_t *device, unsigned char *buffer, unsigned int start,
                            unsigned int length, const memtype_t memtype)
{
    ...
    for(i = 0; i < length; i += flash_block_size)
    {
        ...
        if(memtype == FLASH || memtype == EEPROM || memtype == OPT)
        {
            ...
            { // Device have FLASH_NCR2 register
