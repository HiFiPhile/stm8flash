...
static unsigned int msg_recv_int8(programmer_t *pgm)
{
    return msg_recv_int(pgm, 1);
}
static unsigned int msg_recv_int16(programmer_t *pgm)
{
    return msg_recv_int(pgm, 2);
}
static unsigned int msg_recv_int32(programmer_t *pgm)
{
    return msg_recv_int(pgm, 4);
}
...
void stlink2_init_session(programmer_t *pgm)
{
    int  i;
    char f4_cmd_arg1[] = {
        0x07, 0x07, 0x08, 0x07, 0x04, 0x03, 0x05,
    };
...
{
    stlink2_cmd(pgm, 0xf40a, 8, 0x00, 0x02, 0x00, EX(start), HI(start), LO(start), HI(word), LO(word));
    usleep(2000);
    return (stlink2_get_status(pgm)); // Should be '1'
}
...
int stlink2_write_and_read_byte(programmer_t *pgm, unsigned char byte, unsigned int start)
{
    stlink2_cmd(pgm, 0xf40b, 7, 0x00, 0x01, 0x00, EX(start), HI(start), LO(start), byte);
    usleep(2000);
    stlink2_get_status(pgm);
...
int stlink2_swim_read_range(programmer_t *pgm, const stm8_device_t *device, unsigned char *buffer, unsigned int start,
                            unsigned int length)
{
    ...
    for(i = 0; i < length; i += STLK_READ_BUFFER_SIZE)
    {
        ...
        // Sending USB packet
        stlink2_cmd(pgm, 0xf40b, 6, HI(block_size), LO(block_size), 0x00, EX(block_start), HI(block_start),
                    LO(block_start));
...
int stlink2_swim_write_range(programmer_t *pgm, const stm8_device_t *device, unsigned char *buffer, unsigned int start,
                             unsigned int length, const memtype_t memtype)
{
    ...
    for(i = 0; i < length; i += BLOCK_SIZE)
    {
        if(memtype == FLASH || memtype == EEPROM)
        {
            ...
            stlink2_write_byte(pgm, 0x01,
                               device->regs.FLASH_CR2); // mov 0x01fe, FLASH_CR2; 0x817e - enable write OPT bytes
